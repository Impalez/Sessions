...
void main(void)
{
    ...
    if (desc) {
        ore = 12, minute = 59, secunde = 59;
    }
    else ore = minute = secunde = 0;
...
/***************** JUST a small timer example counting the seconds only - END */
...
#else
        byte leftButton = PORTBbits.RB0;
        byte rightButton = PORTAbits.RA5;
        ...
        if (!desc) {
            ...
            if (secunde == 60)  minute++,   secunde = 0;
            if (minute == 60)   ore++,      minute = 0;

           //if (!leftButton || ore == 12) tick100ms = secunde = minute = ore  = 0;
            if (!leftButton) ore = 12, minute = secunde = 59, desc = 1;
           ...
           if (!secunde) secunde = 59, minute--;
           if (!minute)  minute = 59, ore--;
           
          // if (!leftButton || (!ore && !minute && !secunde)) tick100ms = 0, secunde = minute = 59, ore  = 12;
           if (!leftButton) ore = minute = secunde = 0, desc = 0; 
