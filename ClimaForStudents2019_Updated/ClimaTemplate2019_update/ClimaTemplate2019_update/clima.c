...
#define TEMP_SENS_LM_OFFSET     (400)     // output voltage @ 0*C
...
void updateLcd(void)
{
 ...
 /* display set temperature */
    ...
    {
        /* out temp */
        if (!shouldClearLcd) shouldClearLcd = 1, LcdClear();
        
        sprintf(msg, "%02d", outTemp); // On Board Sensor Temp

        LcdGoTo(0x00+4);
        LcdWriteString(msg);

        /* in temp */
        sprintf(msg, "%02d", inTemp); // Sensor Temp
        LcdGoTo(0x00+12);
        LcdWriteString(msg);
...
void stateMachine(void)
{
    ...
    switch (climaState)
    {
        ...
        {
            /* TODO*/
             if (!shouldClearLcd) shouldClearLcd = 1,   updateLcd();
            ...
            if ( getOnOffButton())//!PORTBbits.RB0)//getOnOffButton())
                climaState = STATE_ON_VENT, shouldClearLcd = 0; 
        ...
        case STATE_ON_COOL:
        {
           ...
           value = (inTemp - setTemp);
            
                       
            //if (value < 0)              climaState = STATE_ON_HEAT, shouldClearLcd = 0;
        //    else 
                if (value == 0)        climaState = STATE_ON_VENT, shouldClearLcd = 0;

           
           setSpeedFanCool(value);
           
           if ( getOnOffButton())//!PORTBbits.RB0)//getOnOffButton())
                climaState = STATE_OFF, shouldClearLcd = 0; 
        ...
        case STATE_ON_HEAT:
        {
            ...
            value = setTemp - inTemp;  
            
            
          // if (value < 0)                  climaState = STATE_ON_COOL, shouldClearLcd = 0;
        //   else
                if (value == 0)                climaState = STATE_ON_VENT, shouldClearLcd = 0;
            
            setSpeedFanHeatVent(value);
        ...
        case STATE_ON_VENT:
        {
            ...
            if (inTemp > setTemp)                            climaState = STATE_ON_COOL, shouldClearLcd = 0;
            else if (inTemp < setTemp)                       climaState = STATE_ON_HEAT, shouldClearLcd = 0;
